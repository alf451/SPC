...
| Runs | `GET/POST /api/runs`, `GET /api/runs/{id}`, `POST /api/runs/{id}/complete` | `RunCreate` accetta anche `work_order_id`/`tool_id` opzionali |
| Measurements | `GET/POST /api/runs/{run_id}/measurements` | POST è per inserimento **manuale**; le misure da strumento arrivano via WS (vedi sotto). Se `tool_position_id` è omesso, usa la posizione attiva del Run (sotto) |
| **v0.6 — Posizione/cavità** | `PUT /api/runs/{id}/current-position` (`{tool_position_id}`, null per nessuna), `POST /api/runs/{id}/skip-position` (`{tool_position_id}`, idempotente), `DELETE /api/runs/{id}/skip-position/{tool_position_id}`, `GET /api/runs/{id}/position-progress` | rilevante solo se il Run ha un `tool_id` collegato (attrezzatura multi-cavità) — `position-progress` restituisce per ogni posizione se è stata saltata e quante misure/osservazioni esistono già per Feature, per confrontarle con `subgroup_size` lato frontend |
| **v0.6 — Tracciabilità** | `GET /api/runs/{id}/traceability`, `PUT /api/runs/{id}/traceability/{campo}` (`{value}`) | campo generico (`traceability_fields`) — "Lotto" seminato dalla migration 0004, altri si aggiungono creando una riga in quella tabella, senza nuovi endpoint |
